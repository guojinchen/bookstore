...
<body>
	...
	<div class="box">
		...
		<div align="right" style="font-size:16px;color:blue; ">
		<form action="findOrder.action" method="post">
		订单号：<input type="text" name="id"/>&nbsp;&nbsp;<input type="submit" value="查询"/> 
		</form>
		<span>${requestScope.message}</span>共${pager.allRow }条记录，共${pager.totalPage }页，当前第${pager.currentPage }页 
		</div>
	<table id="table1" border="1" style="text-align:center;font-size: 14px">
	<tr>
	<th>订单号</th>
	<th>下单时间</th>
	<th>付款类型</th>
	...
	<td>${s.receiverTelephone }</td>
	<td>${s.totalPrice }</td>
	<td><a style="height:20px;width:40px; font-size: 14px" href="delorder.action?orders.orderId=${s.orderId }">删除</a><a style="height:20px;width:40px; font-size: 14px" href="updateorder.action?orders.orderId=${s.orderId }">状态</a>
	
	<a style="height:20px;width:40px; font-size: 14px" href="orderdetail.action?orders.orderId=${s.orderId }">详细</a>
	</td>
	</tr>
	</c:forEach>

	</table>
			<a style="height:20px;width:40px;" href="vieworder.action?page=${pager.currentPage-1 }">上一页</a><a style="height:20px;width:40px"; href="vieworder.action?page=${pager.currentPage+1 }">下一页</a>
		</div>
