...
<body>
	...
	<tr>
	<td>${s[0].bookName }</td>
	<td>${s[0].bookAuthor}</td>
	<td>${s[0].bookPages}</td>
	<td>${s[1]}</td>
	<td ><DIV STYLE="width: 120px; height: 50px; border: 1px solid blue; 
            overflow: hidden; text-overflow:ellipsis"> 
<NOBR>${s[0].authorInfo }</NOBR> 
</DIV></td>
	<td>${s[0].checked }</td>
	<td>${s[0].discountprice }</td>
	<td>${s[0].languages }</td>
	<td>${s[0].price }</td>
	<td>${s[0].publisher }</td>
	<td>${s[0].allsum }</td>
	<td>${s[0].sellsum }</td>	
	<td>${s[0].words }</td>
	<td><a style="height:20px;width:40px; font-size: 14px" href="delbook.action?books.bookId=${s[0].bookId }">删除</a><a style="height:20px;width:40px; font-size: 14px" href="updatebook.action?books.bookId=${s[0].bookId }">更新</a></td>
	</tr>
	</c:forEach>

	</table>
			<a href="viewbook.action?page=${pager.currentPage-1}">上一页</a><a href="viewbook.action?page=${pager.currentPage+1 }">下一页</a>
		</div>
